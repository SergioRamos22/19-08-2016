<?php
	...
	$consulta = "SELECT * from crear_cuenta";
	...
	if($resultado->num_rows > 0 ){
		...
		$tituloReporte = "USUARIOS DE LA PAGINA";
		$titulosColumnas = array('Numero_documento', 'Nombre', 'Apellido','Nombre_de_usuario', 'Contrasenha');
		...
		while ($fila = $resultado->fetch_array()) {
			$objPHPExcel->setActiveSheetIndex(0)
        		    ->setCellValue('A'.$i,  $fila['Numero_documento'])
		            ->setCellValue('B'.$i,  $fila['Nombre'])
        		    ->setCellValue('C'.$i,  $fila['Apellido'])
					->setCellValue('D'.$i,  $fila['Nombre_de_usuario'])
					->setCellValue('E'.$i,  $fila['Contrasenha']);
				 $i++;
		}
